...
fn explain_change_cli_and_mcp_share_summary_builder() {
    ...
    assert_eq!(
        ...
        mcp_explain["changed_file_count"]
    );
    assert_eq!(
        cli_explain["changed_symbol_count"],
        mcp_explain["changed_symbol_count"]
    );
...
fn impact_and_explain_change_share_changed_file_seed_summary() {
    ...
    let impact_changed_paths: std::collections::BTreeSet<String> = impact["analysis"]["base"]
        ["changed_nodes"]
        .as_array()
    ...
    let impact_changed_qnames: std::collections::BTreeSet<String> = impact["analysis"]["base"]
        ["changed_nodes"]
        .as_array()
        .expect("impact changed_nodes array")
        .iter()
