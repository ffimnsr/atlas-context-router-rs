...
CREATE TABLE flow_memberships ( flow_id INTEGER NOT NULL, node_qualified_name TEXT NOT NULL, position INTEGER, role TEXT, extra_json TEXT, PRIMARY KEY (flow_id, node_qualified_name), FOREIGN KEY(flow_id) REFERENCES flows(id) ON DELETE CASCADE, FOREIGN KEY(node_qualified_name) REFERENCES nodes(qualified_name) ON DELETE CASCADE );
