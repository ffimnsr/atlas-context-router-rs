...
/// In-process cache of tree-sitter parse trees, keyed by repo-relative path.
///
...
impl TreeCache {
    ...
    /// Return a reference to the cached tree for `path`, if any.
    pub fn get(&self, path: &str) -> Option<&tree_sitter::Tree> {
        self.trees.get(path)
    }

    /// Store (or replace) the tree for `path`.
    pub fn insert(&mut self, path: String, tree: tree_sitter::Tree) {
        self.trees.insert(path, tree);
    }

    /// Remove and return the tree for `path`.  Callers can move the tree into
    ...
    /// Drop the cached tree for `path`, e.g. when a file is deleted.
