...
fn render_review_change(args: Option<&serde_json::Value>) -> Result<PromptGetResponse> {
    ...
    let text = format!(
        "Use Atlas MCP to review code changes. Stay grounded in tool output only. Prefer graph tools before file search.\n\nTarget inputs:\n- files: {files}\n- base: {base}\n- focus: {focus}\n\nRecommended workflow:\n1. If files are unknown, call detect_changes with base={base}. If files are already known, skip directly to context.\n2. Call get_minimal_context for cheap triage.\n3. Call get_review_context for fuller changed-symbol, neighbor, and risk detail.\n4. Call explain_change when API/signature risk, boundary violations, or test gaps need confirmation.\n5. Call get_impact_radius when blast radius needs explicit changed/impacted nodes and files.\n6. Use query_graph, symbol_neighbors, traverse_graph, or get_context only for targeted follow-up on symbols surfaced by review flow.\n\nResponse requirements:\n- Findings first, ordered by severity.\n- Mention changed symbols, impacted tests, ambiguity, truncation, and confidence limits.\n- Do not invent callers, tests, or dependencies not returned by Atlas."
    );
...
fn render_inspect_symbol(args: Option<&serde_json::Value>) -> Result<PromptGetResponse> {
    ...
    let text = format!(
        "Use Atlas MCP to inspect symbol '{symbol}'. Stay grounded in graph results.\n\nQuestion:\n{question}\n\nRecommended workflow:\n1. Call query_graph with text='{symbol}'. Use semantic=true if name is short or ambiguous.\n2. If multiple candidates appear, compare qname, kind, and file path before choosing. Report ambiguity if unresolved.\n3. Call symbol_neighbors on chosen qname for immediate callers, callees, tests, and local neighborhood.\n4. Call get_context with query='{symbol}' for bounded ranked context. Use intent='usage_lookup' when appropriate.\n5. Call traverse_graph only if one-hop neighbors are insufficient and you need wider caller/callee reach.\n6. Fall back to file reads only after graph tools stop answering structural questions.\n\nResponse requirements:\n- Name exact qname chosen.\n- Separate direct facts from weaker inferences.\n- Mention truncation or unresolved edges when present."
    );
...
fn render_plan_refactor(args: Option<&serde_json::Value>) -> Result<PromptGetResponse> {
    ...
    let text = format!(
        "Use Atlas MCP to plan refactor for target '{target}'. Goal: {goal}. Keep plan deterministic and evidence-backed.\n\nRecommended workflow:\n1. Resolve target with query_graph. If ambiguous, stop and surface ranked candidates.\n2. Call get_context for target-centered context. Prefer intent='refactor_safety', 'rename_preview', or 'dependency_removal' when they match goal.\n3. Call symbol_neighbors for direct callers, callees, tests, and nearby nodes.\n4. Call explain_change or get_impact_radius if likely blast radius crosses files or packages.\n5. Use cross_file_links or concept_clusters when refactor may affect tightly coupled files beyond direct call edges.\n\nResponse requirements:\n- State exact target resolved.\n- List primary risks, affected files/symbols, and test coverage gaps.\n- Recommend validation steps before apply.\n- Do not claim rename/removal safety unless Atlas evidence supports it."
    );
